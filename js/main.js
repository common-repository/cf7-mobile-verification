( function ( $ ) {
	...
	var otp = {
		...
		setOtpInputElementVar: function () {
			...
			otp.mobileOtpHiddenInputEl = otp.setInputElVariables( '#cf7mv-hidden' );
		...
		createMobileInputAndOtherFields: function ( mobileInputName ) {
			var htmlEl = {},
				otpInputEl = '<br><label class="cf7mv-required cf7mv-hide"> OTP (required)<br>\n' +
				'<span class="wpcf7-form-control-wrap cf7mv">' +
				'<input type="number" id="ihs-mobile-otp" name="cf7mv" value="" size="40" class="wpcf7-form-control wpcf7-text wpcf7-validates-as-required" aria-required="true" aria-invalid="false">' +
				'</span>' +
				'</label>',
				sendOtpBtn = '<div class="cf7mv-btn" id="ihs-send-otp-btn">Send OTP</div>',
				submitOtpBtn = '<div class="cf7mv-btn cf7mv-hide" id="ihs-submit-otp-btn">Verify OTP</div>';
		...
		sendOtpAjaxRequest: function ( mobileNumber ) {
			...
			request.done( function ( response ) {
				...
				if ( response.data.otp_pin_sent_to_js ) {
					...
					$( otp.verifyOtpBtnEl ).removeClass( 'cf7mv-hide' );
					$( '.cf7mv-required' ).removeClass( 'cf7mv-hide' );
